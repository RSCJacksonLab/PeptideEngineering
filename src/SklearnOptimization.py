...
def nCV(
    ...
    '''
    Nested cross-validation to estimate test scores/model generalisability after optimisation
    ...
    # outer fold iteration
    kf_outer.get_n_splits(X)
    for k, (trn_idx, tst_idx) in enumerate(tqdm(kf_outer.split(X))):
...
def holdout_CV(    
    ...
    X_trn, X_tst, y_trn, y_tst = train_test_split(X, y, test_size=0.2, random_state=8)
    # perform CV optimisation on training data
    best_set, trn_r2, trn_mae, val_r2, val_mae = hyperparam_opt(
        kf_inner,
        X_trn,
